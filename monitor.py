...
TEST_IGNORE_OCCUPATION = os.getenv("TEST_IGNORE_OCCUPATION", "false").strip().lower() in {"1", "true", "yes", "on"}
...
def get_search_urls() -> list[str]:
    urls = [line.strip() for line in required_env("CROUS_SEARCH_URLS").splitlines() if line.strip()]
    if not urls:
...
def extract_listing(anchor, source_url: str) -> dict | None:
    ...
    title = clean_text(heading.get_text(" ", strip=True)) if heading else clean_text(anchor.get_text(" ", strip=True))
    if not title:
        title = "Logement CROUS disponible"
    price_match = re.search(r"\b\d{2,4}(?:[,.]\d{1,2})?\s*€", text)
    surface_match = re.search(r"\b(?:de\s+)?\d{1,3}(?:[,.]\d+)?(?:\s+à\s+\d{1,3}(?:[,.]\d+)?)?\s*m²", text, flags=re.IGNORECASE)
    address = ""
    for element in card.find_all(["p", "address", "span"]):
        ...
        if re.search(r"\b\d{5}\b", candidate) and len(candidate) <= 180:
            ...
            break
    return {
        "uid": listing_id(url), "title": title[:160], "url": url,
        "price": price_match.group(0) if price_match else "",
        "surface": surface_match.group(0) if surface_match else "",
        "address": address[:220], "details": text[:900], "source": source_url,
    }
...
def parse_bounds(value: str | None) -> list[dict[str, float]] | None:
    ...
    return [{"lon": min_lon, "lat": max_lat}, {"lon": max_lon, "lat": min_lat}]
...
def build_api_body(search_url: str, tool_id: int, page_number: int) -> dict:
    ...
    accessibility = parse_bool(next((value for key, value in query_pairs if key == "accessibility"), None))
    ...
    body: dict = {
        ...
        "sector": search_text, "idTool": tool_id, "occupationModes": selected_modes,
        "equipment": equipment,
        "price": {"min": int(round(price_min * 100)) if price_min is not None else 0,
...
def fetch_search(search_url: str) -> tuple[dict[str, dict], bool]:
    ...
    for page_number in range(1, MAX_PAGES + 1):
        ...
        if page_items:
            ...
            time.sleep(REQUEST_DELAY_SECONDS)
            continue
        logging.warning("HTML CROUS sans annonce : status=%s, bytes=%d, page=%s", response.status_code, len(response.content), page_number)
        break
    if found:
        return found, True
...
def main() -> int:
    ...
    if not state.get("initialized", False):
        ...
        post_discord({"username": "Alerte CROUS", "content": f"✅ Surveillance activée : {len(current)} logement(s) déjà présent(s) mémorisé(s)."})
        return 0
